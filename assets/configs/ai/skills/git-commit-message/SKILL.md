...
git status --porcelain           # what's staged
git diff --staged --stat         # files + churn summary
git diff --staged                # full diff (for complexity assessment)
```

IF no staged changes → stop, tell user to `git add` files first.
