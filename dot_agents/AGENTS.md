...
- **Sandbox Bypass for `gh` CLI**: The default sandbox blocks `gh` CLI commands and skill wrapper scripts due to glob-based path restrictions (`~/.agents/skills/**` is readonly). Always run `gh` commands and skill scripts with `BypassSandbox: true` — no need to attempt them sandboxed first.
