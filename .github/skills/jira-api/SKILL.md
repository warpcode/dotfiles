...
name: jira-api
description: >-
  Query Jira issues, perform JQL searches, inspect transition history and aging metrics,
  and discover project metadata or custom fields. Use when querying Jira, searching tickets
  via JQL, fetching details for specific issue keys (e.g., PROJ-123), or analyzing project workflows.
---

# Jira API

Query and search Jira issues using the bundled `@scripts/main.py` script. This skill is currently **READ-ONLY**.
...
- IF the user's request is vague (e.g., "find the login bugs"), THEN:
  1. Use `python3 .github/skills/jira-api/scripts/main.py fields` to find relevant field names if searching custom fields.
  2. Construct a broad JQL search to find candidate issues.
- IF specific ticket keys are mentioned, THEN use `python3 .github/skills/jira-api/scripts/main.py issues` to fetch full details.
...
Run commands via the bundled `@scripts/main.py` script (`.github/skills/jira-api/scripts/main.py`).

#### JQL Search
Use `jql` for finding issues based on criteria.
```bash
python3 .github/skills/jira-api/scripts/main.py jql "project = PROJ AND status = 'In Progress' ORDER BY updated DESC" --max-results 10
```
...
python3 .github/skills/jira-api/scripts/main.py jql "project = PROJ" --max-results 10 --param nextPageToken=YOUR_TOKEN_HERE
```
...
```bash
python3 .github/skills/jira-api/scripts/main.py issues PROJ-123 PROJ-456 --fields "summary,status,description,assignee,priority,comment"
```
...
python3 .github/skills/jira-api/scripts/main.py issues PROJ-123 --expand changelog

# Fetch history for all issues matching a search
python3 .github/skills/jira-api/scripts/main.py jql "project = PROJ AND status = Blocked" --expand changelog
```
...
```bash
python3 .github/skills/jira-api/scripts/main.py fields "Story Points"
```

#### Schema & Metadata Discovery
Use these subcommands to understand the Jira instance structure.
- **Statuses**: `python3 .github/skills/jira-api/scripts/main.py statuses` (Returns map of ID -> {name, category})
- **Issue Types**: `python3 .github/skills/jira-api/scripts/main.py types` (Returns map of ID -> {name, subtask})
- **Priorities**: `python3 .github/skills/jira-api/scripts/main.py priorities`
- **Resolutions**: `python3 .github/skills/jira-api/scripts/main.py resolutions`
- **Projects**: `python3 .github/skills/jira-api/scripts/main.py projects`
- **Users**: `python3 .github/skills/jira-api/scripts/main.py users "John Doe"` (Search for users by name or email)
...
python3 .github/skills/jira-api/scripts/main.py call GET "/rest/api/3/issue/PROJ-123/transitions"
```
...
1. **Discover**: Run `python3 .github/skills/jira-api/scripts/main.py statuses` to identify which statuses map to `statusCategory: In Progress` but represent "Passive/Waiting" states in this project.
