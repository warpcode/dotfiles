...
while [[ $# -gt 0 ]]; do
  case $1 in
    -h|--help)
      echo "Usage: ./add_comment_to_pending_review.sh [OPTIONS]"
      echo ""
      echo "Add a review comment to an existing pending pull request review."
      echo ""
      echo "Options:"
      echo "  --owner <value> (Required)"
      echo "  --repo <value> (Required)"
      echo "  --pull-number <value> (Required)"
      echo "  --review-id <value> (Required)"
...
if [[ -z "$review_id" ]]; then
  echo "Error: --review-id is required. Use --help for usage." >&2
  exit 1
fi
if [[ -z "$body" ]]; then
...
gh api -X POST repos/"$owner"/"$repo"/pulls/"$pull_number"/reviews/"$review_id"/comments -f body="$body"
