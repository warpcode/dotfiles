...
PR_INFO=$(gh pr view "$PR_NUMBER" --json headRefName,baseRefName,reviewDecision,mergeable,statusCheckRollup 2>/dev/null)
if [[ -z "$PR_INFO" ]]; then
...
BASE_BRANCH=$(jq -r '.baseRefName // empty' <<< "$PR_INFO")
