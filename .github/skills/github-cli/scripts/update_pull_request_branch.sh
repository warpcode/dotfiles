...
while [[ $# -gt 0 ]]; do
  case $1 in
    -h|--help)
      echo "Usage: ./update_pull_request_branch.sh [OPTIONS]"
      echo ""
      echo "Update a pull request branch by merging or rebasing the latest changes from the base branch."
      echo ""
      echo "Options:"
...
gh pr update-branch "$pull_number" --repo "$owner"/"$repo"
