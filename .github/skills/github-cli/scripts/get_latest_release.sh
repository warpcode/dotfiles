...
while [[ $# -gt 0 ]]; do
  case $1 in
    -h|--help)
      echo "Usage: ./get_latest_release.sh [OPTIONS]"
      echo ""
      echo "Retrieve the latest published release, tag name, notes, and assets for a repository."
      echo ""
      echo "Options:"
...
gh release view --repo "$owner"/"$repo" --json name,tagName,body
