...
while [[ $# -gt 0 ]]; do
  case $1 in
    -h|--help)
      echo "Usage: ./delete_file.sh [OPTIONS]"
      echo ""
      echo "Delete a file from a remote repository branch using the GitHub Contents API."
      echo ""
      echo "Options:"
...
gh api -X DELETE repos/"$owner"/"$repo"/contents/"$path" -f message="$message" -f branch="$branch" -f sha="$sha"
